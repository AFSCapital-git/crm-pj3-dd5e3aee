...
export const listEditais = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("linhas_editais_finep").select("*").order("prazo_submissao", { ascending: true, nullsFirst: false });
  ...
  .handler(async ({ data, context }) => {
    if (data.id) {
      const { data: row, error } = await context.supabase
        .from("linhas_editais_finep").update(data.values).eq("id", data.id).select().single();
      if (error) throw error;
      return row;
    }
    const { data: row, error } = await context.supabase
      .from("linhas_editais_finep").insert(data.values).select().single();
    if (error) throw error;
    return row;
  });
...
export const deleteEdital = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("linhas_editais_finep").delete().eq("id", data.id);
    if (error) throw error;
    return { ok: true };
  });
