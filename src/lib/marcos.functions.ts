...
const marcoSchema = z.object({
  ...
  tipo: z.enum(["relatorio_tecnico","relatorio_financeiro","prestacao_contas_parcial","prestacao_contas_final"]),
  ...
  .handler(async ({ data, context }) => {
    if (data.id) {
      const { data: row, error } = await context.supabase
        .from("marcos_entregas").update(data.values).eq("id", data.id).select().single();
      if (error) throw error;
      return row;
    }
    const { data: row, error } = await context.supabase
      .from("marcos_entregas").insert(data.values).select().single();
    if (error) throw error;
    return row;
  });
...
export const createInteracao = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    projeto_id: z.string().uuid(),
    tipo: z.enum(["reuniao","email","ligacao","alteracao_cronograma","aditivo_contratual","nota"]),
