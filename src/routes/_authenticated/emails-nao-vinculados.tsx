...
import { listEmailsPendentes, vincularEmailManual, descartarEmailPendente } from "@/lib/emails.functions";
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          E-mails encaminhados sem código de rastreio válido no assunto. Vincule ao projeto correto ou descarte.
        </p>
      </div>

      {q.isLoading && <p className="text-sm text-muted-foreground">Carregando…</p>}
      {!q.isLoading && (q.data?.length ?? 0) === 0 && (
        <Card><CardContent className="py-8 text-center text-sm text-muted-foreground">Fila vazia.</CardContent></Card>
      )}
          ...
          <Card key={p.id}>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <Badge variant="outline" className="ml-auto text-xs">{p.motivo}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="text-xs text-muted-foreground">
                De <span className="text-foreground font-medium">{p.remetente_original}</span> ·{" "}
                {p.data_email_original ? new Date(p.data_email_original).toLocaleString("pt-BR") : new Date(p.criado_em).toLocaleString("pt-BR")}
              </div>
              {p.corpo_texto && (
                <p className="text-sm whitespace-pre-wrap text-muted-foreground line-clamp-4">{p.corpo_texto}</p>
              )}
              <div className="flex items-center gap-2">
                <Select value={sel[p.id] ?? ""} onValueChange={(v) => setSel({ ...sel, [p.id]: v })}>
                  <SelectTrigger className="max-w-md"><SelectValue placeholder="Selecionar projeto…" /></SelectTrigger>
                  <SelectContent>
                    {projetos.data?.map((pr: any) => (
                      <SelectItem key={pr.id} value={pr.id}>
                        [{pr.codigo_rastreio}] {pr.nome_projeto}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
