  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Todos os marcos ordenados por data prevista.</p>
      </div>
        ...
        <div className="w-64">
          <Select value={consultor} onValueChange={setConsultor}>
            ...
            <SelectContent>
              ...
              {(qU.data ?? []).map((u: any) => <SelectItem key={u.id} value={u.id}>{u.nome}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div className="w-56">
      ...
      <Card>
        <CardHeader><CardTitle>{filtered.length} marcos</CardTitle></CardHeader>
        <CardContent>
          {q.isLoading ? <p>Carregando…</p> : (
            <div className="divide-y">
              {filtered.map((m: any) => (
                <Link key={m.id} to="/projetos/$id" params={{ id: m.projeto_id }} className="flex items-center justify-between py-3 hover:bg-muted/40 -mx-2 px-2 rounded">
              ...
              {filtered.length === 0 && <p className="text-sm text-muted-foreground py-6 text-center">Nenhum marco no filtro atual.</p>}
            </div>
          )}
        </CardContent>
      </Card>
