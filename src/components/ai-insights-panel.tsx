...
export function AiInsightsPanel({ projetoId, isAdmin = false }: { projetoId?: string; isAdmin?: boolean }) {
  ...
  const mGenPortfolio = useMutation({
    ...
    onSuccess: () => { toast.success("Análise gerada pela IA"); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
  const mGenReport = useMutation({
    mutationFn: () => genReportFn({ data: { projeto_id: projetoId! } }),
    onSuccess: () => { toast.success("Rascunho gerado pela IA"); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
  const mReview = useMutation({
    mutationFn: (v: { id: string; aprovado: boolean }) => reviewFn({ data: v }),
    onSuccess: () => { toast.success("Revisão registrada"); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
  const mDel = useMutation({
    mutationFn: (id: string) => delFn({ data: { id } }),
    onSuccess: () => { toast.success("Insight removido"); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
    ...
    <Card className="border-primary/30">
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Todo conteúdo abaixo é <strong>gerado por IA</strong> e precisa de revisão humana antes de virar ação.
          </p>
        </div>
        <div className="flex gap-2">
          {projetoId ? (
            <Button size="sm" onClick={() => mGenReport.mutate()} disabled={mGenReport.isPending}>
              {mGenReport.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Sparkles className="h-4 w-4 mr-2" />}
              Gerar rascunho de relatório
            </Button>
          ) : (
            <Button size="sm" onClick={() => mGenPortfolio.mutate()} disabled={mGenPortfolio.isPending}>
              {mGenPortfolio.isPending ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Sparkles className="h-4 w-4 mr-2" />}
              Analisar portfólio
            </Button>
          )}
        </div>
      ...
      <CardContent>
        {q.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">Nenhum insight gerado ainda. Clique no botão acima para pedir uma análise.</p>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      ...
                      {it.modelo && <> · modelo <code className="text-[10px]">{it.modelo}</code></>}
                    </p>
                  </div>
                  <div className="flex gap-1 shrink-0">
                    {!it.revisado_por_humano && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => mReview.mutate({ id: it.id, aprovado: true })}>
                          <CheckCircle2 className="h-4 w-4 mr-1" /> Aprovar
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => mReview.mutate({ id: it.id, aprovado: false })}>
                          <XCircle className="h-4 w-4 mr-1" /> Rejeitar
                        </Button>
                      </>
                    )}
                    {isAdmin && (
                      <Button size="icon" variant="ghost" onClick={() => confirm("Excluir insight?") && mDel.mutate(it.id)}>
